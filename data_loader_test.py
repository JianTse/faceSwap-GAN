    ...
    def create_tfdata_iter(self, filenames, fns_all_trn_data, batch_size, dir_bm_eyes, resolution, 
        ...
        dataset = dataset.apply(
            tf.contrib.data.map_and_batch(
                lambda filenames: tf.py_func(  # py_func  py_function
                    func=read_image, 
                    inp=[filenames, 
                         fns_all_trn_data, 
                         dir_bm_eyes, 
...
def test():
    ...
    train_A_list_tmp = getDirFnPath(img_dirA)
    ...
    while gen_iterations <= TOTAL_ITERS:
        ...
        warped_A = (warped_A + 1) * 127.5
        target_A = (target_A + 1) * 127.5
        bm_eyes_A = (bm_eyes_A) * 255
        warped_A = np.clip(warped_A,0,255)
        target_A = np.clip(target_A, 0, 255)
        bm_eyes_A = np.clip(bm_eyes_A, 0, 255)
        warped_A = warped_A.astype(np.uint8)
        target_A = target_A.astype(np.uint8)
        bm_eyes_A = bm_eyes_A.astype(np.uint8)
        cv2.imshow('warp', warped_A[0])
        cv2.imshow('target', target_A[0])
        cv2.imshow('bm_eye', bm_eyes_A[0])
        cv2.waitKey(0)
        print('step3')
test()