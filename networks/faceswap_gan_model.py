...
class FaceswapGANModel():
    ...
    def build_decoder(nc_in=512, 
        ...
        out = concatenate([alpha, bgr])  # 解码的时候会生成一个mask和一张生成图
        outputs.append(out)
    ...
    def define_variables(netG):
        distorted_input = netG.inputs[0]  #原始图
        fake_output = netG.outputs[-1]    # 生成图+mask
        alpha = Lambda(lambda x: x[:,:,:, :1])(fake_output) # 生成的mask
        bgr = Lambda(lambda x: x[:,:,:, 1:])(fake_output)  # 生成的图像

        masked_fake_output = alpha * bgr + (1-alpha) * distorted_input   # 最终合成的生成图，

        fn_generate = K.function([distorted_input], [masked_fake_output]) # 输入原始图，输出最终合成结果
        fn_mask = K.function([distorted_input], [concatenate([alpha, alpha, alpha])])  # 输入原始图，输出mask
        fn_abgr = K.function([distorted_input], [concatenate([alpha, bgr])]) # 输入原始图，输出结果图+mask
        fn_bgr = K.function([distorted_input], [bgr])# 输入原始图，输出结果图
    ...
    def train_one_batch_G(self, data_A, data_B):
        ...
        errGA = self.netGA_train([warped_A, target_A, bm_eyes_A])  #丢进去扭曲图、原始图、mask
    ...
    def train_one_batch_D(self, data_A, data_B):
        if len(data_A) == 4 and len(data_B) == 4:
            ...
            raise ValueError("Something's wrong with the input data generator.")
        errDA = self.netDA_train([warped_A, target_A])  #丢进去扭曲图、原始图
        errDB = self.netDB_train([warped_B, target_B])
        return errDA, errDB
    
    def transform_A2B(self, img):
        return self.path_abgr_B([[img]])  # 丢进去一张图，用decodeB输出生成图+mask
    
    def transform_B2A(self, img):
        return self.path_abgr_A([[img]])  # 丢进去一张图，用decodeA输出生成图+mask